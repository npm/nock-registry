...
function packPackageToStream (manifest, reg) {
  ...
  if (bundledDeps) {
    ...
    bundledDeps.forEach(d => {
      ...
      pack.entry({ name: `${unscopedName}/node_modules/${d.name}/index.js` },
        `console.log('Hello from ${d.name}@${d.version}!')`
      )
    })
  }
 ...
 *
 * Only simple graphs are currenly supported, the following features will come in further
 * iterations:
 * - lockfile
 * - shinkwrap
